...
void worker(int threadId) {
    ...
    // Push all local results to global results
    lock_guard<mutex> lock(mtx);
    primeResults.insert(primeResults.end(), localResults.begin(), localResults.end());
}
...
int main() {
    ...
    cout << "\n[Variant 4] Multi-threaded prime search with batch printing and individual number testing\n";
    ...
    // Print all results after threads finish (Variant 4 requirement: batch printing)
    cout << "\n=== All primes found (" << primeResults.size() << " numbers) ===\n";
    for (const auto &result : primeResults) {
        cout << result << endl;
    }

    cout << "\nExecution time: " << duration.count() << " ms" << endl;
    cout << "Run finished at: " << getTimestamp() << endl;
